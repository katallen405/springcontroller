...
move_status wait, without needing rosbridge, gen3_torque_control,
...
def node():
    ...
    n._move_wait_timeout_sec = 1.0  # keep the timeout test fast
    yield n
    n.destroy_node()
...
def test_success_when_clear(node):
    ...
    ok, msg = node._move_to_joint_angles(ANGLES)
    assert ok is True
    assert msg == "Moved to target."


def test_reports_non_succeeded_terminal_status(node):
    node._call_sync = lambda client, req, timeout_sec=None: _CheckResp(in_collision=False, in_danger=False)
...
def test_times_out_if_move_status_never_updates(node):
    ...
    assert elapsed < 3.0  # bounded by node._move_wait_timeout_sec (1.0s), not hanging
